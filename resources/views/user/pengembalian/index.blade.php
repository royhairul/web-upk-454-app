  ...
  <div class="container max-w-full min-h-full">
    ...
    <div class="p-8 w-full">
      <header class="flex gap-x-40 align-center">
        <div>
          <h2 class="text-2xl font-semibold leading-8">Pengembalian</h2>
          <p class="text-sm text-gray-500">Pengembalian yang anda ajukan</p>
        </div>
      </header>
      <main class="mt-5">

        @if (sizeOf($pengembalian) == 0)
        ...
        <ul role="list" class="flex flex-col divide-y-2 divide-gray-200 gap-4 divide-dotted bg-white p-5 rounded-md">
          ...
          <a href="{{ route('pjmk.kembali.detail', $p->pengembalian_id) }}">
          <li class="flex justify-between gap-x-6 p-3 group hover:bg-gray-100 transition-colors duration-300 cursor-pointer">
              ...
              <div class="shrink-0">
                  ...
                  @if ($p->pengembalian_status == 'Waiting') ring-gray-500/10 bg-gray-50 text-gray-400
                  @elseif ($p->pengembalian_status == 'Disetujui') ring-green-600/10 bg-green-50 text-green-700
                  @elseif ($p->pengembalian_status == 'Ditolak') ring-rose-500/10 bg-rose-50 text-rose-600
                  @endif
                  ">
                    <span class="material-symbols-outlined block">
                      @if ($p->pengembalian_status == 'Waiting') pending
                      @elseif ($p->pengembalian_status == 'Disetujui') check_circle
                      @elseif ($p->pengembalian_status == 'Ditolak') cancel
                      @endif
                    </span>
                    <span class="block text-sm">{{ $p->pengembalian_status }}</span>
                  </span>
              </div>
            </li>
          </a>
